...
$Runner = ".\日本語版コード\05e_TOEIC先行研究準拠_OpenAI_Gemini_Claude実験.py"
...
$SmokeCompletionReport = ".\日本語版データ\TOEIC\07_本番前チェック\03_OpenAI_Gemini_Claude研究完了チェック.json"
$CompletionReport = ".\日本語版データ\TOEIC\07_本番前チェック\04_先行研究準拠研究完了チェック.json"
...
Write-Host "E-GEO 日本語TOEIC研究：先行研究準拠 OpenAI + Gemini + Claude" -ForegroundColor Green
Write-Host "Mode: $Mode"
Write-Host "Hard stop: USD $HardStopUsd"
Write-Host "学習Re-ranker: GPT-4.1 / Gemini"
Write-Host "最終Test評価: GPT-5 / Gemini / Claude"
...
Write-Host "Re-rankerは原著のモデルファミリー別System Promptを使用します。"
...
if ($Mode -eq "Validate") {
    Invoke-Step "6. 先行研究準拠設定と3社モデル設定を検査（API呼び出し0回）" {
...
if ($Mode -eq "Smoke") {
    Invoke-Step "8. 先行研究準拠の3社smoke実験" {
        uv run python $Runner `
            --mode smoke `
            --experiment-config $ExperimentConfig `
            --execute `
...
if (-not $SkipSmoke) {
    Invoke-Step "8. 正式実験前の先行研究準拠3社smoke実験" {
        uv run python $Runner `
            --mode smoke `
            --experiment-config $ExperimentConfig `
            --execute `
...
Invoke-Step "9. 15プロンプトの正式メタ最適化・3モデルTest評価" {
    uv run python $Runner `
        --mode full `
        --experiment-config $ExperimentConfig `
        --execute `
...
Invoke-Step "11. 先行研究準拠の研究完了チェック" {
...
Write-Host "先行研究準拠の実行・統計・埋め込み分析が終了しました。" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
...
Write-Host "10特徴を0/1/2で入力後、手順10・11を再実行すると全分析完了になります。"
