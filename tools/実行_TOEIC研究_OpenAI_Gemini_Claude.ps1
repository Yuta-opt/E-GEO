...
if ($Mode -eq "Validate") {
    Invoke-Step "7. 3社API接続Smokeの設定検査（API呼び出し0回）" {
...
Invoke-Step "7. 3種類のAPIキー存在確認（API呼び出し0回）" {
    uv run python $ConnectivitySmoke `
        --experiment-config $ExperimentConfig `
        --model-profile $ModelProfile `
        --output-dir $ConnectivityDir `
...
Invoke-Step "9. Rewriter・Meta・全Re-rankerの少額3社API接続Smoke" {
    uv run python $ConnectivitySmoke `
        --experiment-config $ExperimentConfig `
        --model-profile $ModelProfile `
        --output-dir $ConnectivityDir `
...
Invoke-Step "10. OpenAIで候補10件を全80購入意図について選定" {
    uv run python $CandidateRunner `
        --model $CandidateModel `
        --execute
}
...
if ($Mode -eq "Smoke") {
    Invoke-Step "11. 費用配分版の3社正式工程Smoke" {
        uv run python $Runner `
            --mode smoke `
            --experiment-config $ExperimentConfig `
            --model-profile $ModelProfile `
...
if (-not $SkipSmoke) {
    Invoke-Step "11. 正式実験前の3社工程Smoke" {
        uv run python $Runner `
            --mode smoke `
            --experiment-config $ExperimentConfig `
            --model-profile $ModelProfile `
...
Invoke-Step "14. 3社費用配分版の最終安全完了チェック" {
...
Write-Host "再実行時は成功済みAPIジョブ、共有リライト、埋め込みをキャッシュから再利用します。"
