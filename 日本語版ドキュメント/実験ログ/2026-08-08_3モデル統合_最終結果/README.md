...
7. `07_英語先行研究_vs_日本語TOEIC_改善セル比較.svg`
   - 対応データ: `07_英語先行研究_vs_日本語TOEIC_改善セル比較.csv`

1〜3はClaude込みの3モデル版です。4〜5は補助分析です。

6はGPT-5 / Geminiの平均順位の変化を参考表示する図です。研究間で条件が異なるため、絶対値そのものの比較には使いません。

7は**先行研究と本研究を同じ数え方で比べる主比較図**です。1セルを「1種類の指示文 × 1つの順位評価モデル」とし、改善後に順位が良くなったセル数を比較します。

- 先行研究（英語）: 63 / 75セル = 84.0%
- 本研究（日本語TOEIC）: 35 / 45セル = 77.8%

この比較から、英語・日本語ともに大半の組み合わせで、自動改善後の順位が良くなったことを示します。
